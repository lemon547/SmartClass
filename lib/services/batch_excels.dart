...
class AttendanceExcelRow {
  const AttendanceExcelRow({
    required this.date,
    this.studentNo = '',
    this.name = '',
...
abstract final class AttendanceBatchExcel {
  ...
  static Uint8List template() => ExcelBatchHelper.buildTemplate(
        ...
        headers: headers,
        sampleRows: [
          ['2026-07-17', '20250101', '张三', '出勤', '示例，可删除'],
          ['2026-07-17', '20250102', '李四', '请假', ''],
        ],
        tips: [
          ('日期', 'yyyy-MM-dd，如 2026-07-17'),
          ('学号', '优先按学号匹配学生'),
          ('姓名', '无学号时按姓名匹配'),
          ('状态', '出勤 / 迟到 / 请假 / 缺勤'),
          ('备注', '选填'),
  ...
  static ({List<AttendanceExcelRow> rows, List<String> warnings}) parse(
    Uint8List bytes,
  ) {
    ...
    final cols = ExcelBatchHelper.mapHeaders(sh, aliases);
    final out = <AttendanceExcelRow>[];
    for (var r = 1; r < sh.maxRows; r++) {
      ...
      final statusRaw = ExcelBatchHelper.at(row, cols, '状态');
      final remark = ExcelBatchHelper.at(row, cols, '备注');
      if (date.isEmpty && no.isEmpty && name.isEmpty) continue;
      if (ExcelBatchHelper.isSampleMarker(remark) && name == '张三') continue;
      if (date.isEmpty) {
        warnings.add('第 ${r + 1} 行缺日期，已跳过');
        continue;
      }
      final status = _parseStatus(statusRaw);
      if (status == null) {
        warnings.add('第 ${r + 1} 行状态无法识别：$statusRaw');
        continue;
      }
      out.add(AttendanceExcelRow(
        date: date,
        studentNo: no,
        name: name,
        status: status,
  ...
  static AttendanceStatus? _parseStatus(String raw) {
    final t = raw.trim();
    return switch (t) {
      '出勤' || '到' || '正常' || 'present' => AttendanceStatus.present,
      '迟到' || 'late' => AttendanceStatus.late,
      '请假' || 'leave' => AttendanceStatus.leave,
      '缺勤' || '旷课' || 'absent' => AttendanceStatus.absent,
      _ => null,
    };
  }
}
...
class TimetableExcelRow {
  const TimetableExcelRow({
...
abstract final class TimetableBatchExcel {
  static const sheet = '课程表';
  static const headers = ['星期', '节次', '科目'];
  ...
  static ({List<TimetableExcelRow> rows, List<String> warnings}) parse(
    Uint8List bytes,
  ) {
    final excel = Excel.decodeBytes(bytes);
    final sh = ExcelBatchHelper.pickSheet(excel, preferred: sheet);
    ...
    final out = <TimetableExcelRow>[];
    for (var r = 1; r < sh.maxRows; r++) {
      final row = sh.row(r);
      final wd = ExcelBatchHelper.parseWeekday(
        ExcelBatchHelper.at(row, cols, '星期'),
      );
      final period = int.tryParse(ExcelBatchHelper.at(row, cols, '节次'));
      final subject = ExcelBatchHelper.at(row, cols, '科目');
      if (wd == null && period == null && subject.isEmpty) continue;
      if (subject.contains('示例') && subject.contains('可删除')) continue;
      if (wd == null || period == null || period < 1) {
        warnings.add('第 ${r + 1} 行星期/节次无效，已跳过');
        continue;
      }
      out.add(TimetableExcelRow(weekday: wd, period: period, subject: subject));
    }
    return (rows: out, warnings: warnings);
  }
}
